  ...
  return (
    <div className="cartList">
      ...
      {itemList.length === 0 && (
        <h1 className="myH1">
          {" "}
          {orderStatus.res !== null ? orderStatus.res : "No Order yet"}
        </h1>
      )}
      <h1>Sum: {sum}</h1>
      {itemList.length > 0 && (
        <div className="submit">
          <div>
            <h1
              className="subCartList"
              style={{ color: "white", marginRight: "10px" }}
            >
              Total: {sum}
            </h1>
          </div>
          <div className="subSubmit" onClick={myOrder}>
            {orderStatus.loading === false ? "Place Order" : "Loading..."}
          </div>
        </div>
      )}
      ...
      {hideNum && (
        <div className="room_table_number">
          ...
          <input
            type="text"
            placeholder="#..."
            onChange={(e) => setRoomOrTable_number(e.target.value)}
          />
        </div>
      )}
    </div>
